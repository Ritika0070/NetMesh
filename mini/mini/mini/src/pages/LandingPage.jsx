...
function NetworkMesh3D() {
  ...
  useEffect(() => {
    ...
    function buildNodes() {
      nodes = Array.from({ length: NODE_COUNT }, () => {
        ...
        return {
          ...
          cx: Math.random() * (width || 800),
          cy: Math.random() * (height || 600),
    ...
    function project(node) {
      const tiltX = (mouse.x / (width  || 1) - 0.5) * 0.4;
      const tiltY = (mouse.y / (height || 1) - 0.5) * 0.4;
      node.orbitAngle += node.orbitSpeed;
      const ox = Math.cos(node.orbitAngle) * node.orbitRadius;
      const oy = Math.sin(node.orbitAngle * 0.7) * node.orbitRadius * 0.6;
      return {
        px: node.cx + ox + tiltX * (1 - node.z) * 80,
        py: node.cy + oy + tiltY * (1 - node.z) * 80,
      };
    }
    ...
    function draw(t) {
      ...
      for (let i = 0; i < projected.length; i++) {
        for (let j = i + 1; j < projected.length; j++) {
          ...
          if (dist < MAX_DIST * avgZ) {
            const alpha = (1 - dist / (MAX_DIST * avgZ)) * avgZ * 0.55;
            const pulse = 0.6 + 0.4 * Math.sin(t * 0.001 + a.node.pulsePhase);
            ctx.beginPath();
            ctx.moveTo(a.px, a.py);
            ctx.lineTo(b.px, b.py);
            ctx.strokeStyle = `rgba(${Math.round(120+avgZ*80)},${Math.round(80+avgZ*80)},${Math.round(20+avgZ*20)},${alpha*pulse})`;
      ...
      <div className="sv-profile-card">
        <div className="sv-avatar">
          <svg viewBox="0 0 40 40" fill="none" width="26" height="26">
            <circle cx="20" cy="15" r="7" stroke="#C4A050" strokeWidth="1.5"/>
            <path d="M6 36c0-7.732 6.268-14 14-14s14 6.268 14 14" stroke="#C4A050" strokeWidth="1.5" strokeLinecap="round"/>
          </svg>
        </div>
    ...
    <div className="lp-step-visual sv-chat-visual">
      <div className="sv-chat-header">
        ...
        <div>
          ...
          <div className="sv-chat-status">
            <span className="sv-online-dot" /> online
          </div>
        </div>
      </div>
      <div className="sv-chat-messages">
        ...
        <div className="sv-msg sv-msg--out sv-msg--action" style={{ "--d": "2s" }}>
          <svg width="11" height="11" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" style={{ marginRight: 5 }}>
        ...
        <div className="lp-steps">
          ...
          <div className="lp-step">
            ...
            <div className="lp-step-body">
              ...
              <p className="lp-step-desc">Enter a session code provided by the event organizer, or create your own session with a custom topic and requirement. Sessions are ephemeral and auto-expire after the event ends, keeping your data clean.</p>
              <div className="lp-step-tags">
          ...
          <div className="lp-step">
            ...
            <div className="lp-step-body">
              ...
              <p className="lp-step-desc">Once inside a session, the recommendation engine analyzes all attendees and ranks them by compatibility with your interests and goals. You see a live-ranked list of people worth talking to, updated in real time.</p>
              <div className="lp-step-tags">
